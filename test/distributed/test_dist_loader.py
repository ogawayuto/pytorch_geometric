...
def create_dist_data(tmp_path, rank):
    ...
    if feat_store.meta['is_hetero']:
        input_nodes = torch.cat([feat_store.get_global_id(ntype) for ntype in feat_store.meta['node_types']])
    else:
        input_nodes = feat_store.get_global_id(None)
    return data, input_nodes

def dist_neighbor_loader(
    tmp_path: str,
    world_size: int,
    rank: int,
    master_addr: str,
...
def test_dist_neighbor_loader_homo(
    ...
    w0 = mp_context.Process(target=dist_neighbor_loader, args=(
        tmp_path, num_parts, 0, addr, port, num_workers, concurrency, async_sampling))

    w1 = mp_context.Process(target=dist_neighbor_loader, args=(
