...
class DistNeighborSampler():
  ...
  #) -> NeighborOutput:
    ...
    #print(f"\n\n\n\n XXX.0---- DistNSampler: async _sample_one_hop(), one_hop_num={one_hop_num}, pidx={self.dist_graph.partition_idx}, srcs.size(0)={srcs.size(0)}, srcs={srcs}, batch={batch}, seed_time={seed_time} ------")
    ...
    #print(f"\n\n\n\n XXX.1---- DistNSampler: async _sample_one_hop(), one_hop_num={one_hop_num}, pidx={self.dist_graph.partition_idx}, partition_ids={partition_ids} ------")
    ...
    for i in range(self.dist_graph.num_partitions):
      ...
      if p_srcs.shape[0] > 0:
        ...
        if p_id == self.dist_graph.partition_idx:
          ...
          #print(f"----000---- _sample_one_hop(): Node{self.dist_graph.partition_idx}-LOCAL---------, pidx={self.dist_graph.partition_idx}, p_id={p_id}, p_nodes.shape={p_nodes.shape}, p_nodes={p_nodes} ------")
          p_nbr_out = self._sampler.sample_one_hop(p_srcs, one_hop_num, p_seed_time, p_batch, src_etype)
          #print(f"----000.2---- _sample_one_hop(): Node{self.dist_graph.partition_idx}-LOCAL---------, pidx={self.dist_graph.partition_idx}, p_nbr_out={p_nbr_out}, node.size={p_nbr_out.node.size()}, edge.size=={p_nbr_out.edge.size()}, meta_len ={len(p_nbr_out.metadata)}----\n\n")
          partition_results.pop(p_id)
          partition_results.insert(p_id, PartialNeighborOutput(p_nodes, p_nbr_out))
        else:
          #print(f"----111---- _sample_one_hop(): Node{self.dist_graph.partition_idx}- REMOTE --------, pidx={self.dist_graph.partition_idx}, p_id={p_id}, p_nodes.shape={p_nodes.shape}, p_nodes={p_nodes}  ------")
          ...
          #print(f"----111.2---- _sample_one_hop(): Node{self.dist_graph.partition_idx}- REMOTE --------, pidx={self.dist_graph.partition_idx}, temp_futs={temp_futs.wait()}, node.size={temp_futs.wait().node.size()}, edge.size=={temp_futs.wait().edge.size()}, meta_len ={len(temp_futs.wait().metadata)} ------\n\n")
    ...
    if len(remote_nodes) == 0:
      print(f"--------NO REMOTE NEIGHBORS !!!!!!! ----one_hop_num={one_hop_num}, pidx={self.dist_graph.partition_idx}, partition_results ={partition_results}---  ")
    ...
    #print(f"\n\n-------55555---- one_hop_num={one_hop_num}, pidx={self.dist_graph.partition_idx}, -- DistNSampler: _sample_one_hop() before stitching -----------------  partition_results={partition_results}-------")
