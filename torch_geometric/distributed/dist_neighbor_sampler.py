...
class RpcSubGraphCallee(RPCCallBase):
    ...
    def rpc_async(self, *args, **kwargs):
        ...
        output = self.sampler.subgraph_op.node_subgraph(
            args[0].to(self.device), with_edge)
        eids = output.eids.to('cpu') if with_edge else None
        return output.nodes.to('cpu'), output.rows.to('cpu'), output.cols.to(
            'cpu'), eids
...
class DistNeighborSampler:
    ...
    def __init__(self,
        ...
        assert isinstance(
            self.dist_graph, LocalGraphStore), f"Provided data is in incorrect format: self.dist_graph must be `LocalGraphStore`, got {type(self.dist_graph)}"
        assert isinstance(
            self.dist_feature, LocalFeatureStore), f"Provided data is in incorrect format: self.dist_feature must be `LocalFeatureStore`, got {type(self.dist_feature)}"
    ...
    def sample_from_nodes(
        ...
        **kwargs
        ...
        self.event_loop.add_task(coro=self._sample_from(
            self.node_sample, inputs), callback=cb)
        # print(f"--------- TTT.3------------")
        return None
        ...
        if self.channel is None:
            return self.event_loop.run_task(
                coro=self._sample_from(
                    edge_sample_async, inputs, self.node_sample, self._sampler.
                    num_nodes, self.disjoint, self._sampler.node_time,
    ...
    async def node_sample(
        ...
        if self.is_hetero:
            ...
            # if self.disjoint else None
            batch_dict = BatchDict(self._sampler.node_types)
            ...
            for i in range(self._sampler.num_hops):
                ...
                for etype in edge_types:
                    ...
                    one_hop_num = self.num_neighbors[i] if isinstance(
                        self.num_neighbors, List) else self.num_neighbors[etype][i]

                    task_dict[etype] = self.event_loop._loop.create_task(self._sample_one_hop(
                        node_dict.src[src], one_hop_num, seed_time, batch_dict.src[src], etype))
                ...
                for etype, task in task_dict.items():
                    ...
                    node_wo_dupl = OrderedSet((out.node).tolist()) if not self.disjoint else OrderedSet(
                    ...
                    node_dict.src[dst] = Tensor(
                        node_wo_dupl
                        if not self.disjoint else list(zip(*node_wo_dupl))
                        [1]).type(
                        torch.int64)
            ...
            row_dict, col_dict = torch.ops.pyg.hetero_relabel_neighborhood(
                node_types, edge_types, {input_type: seed},
                node_dict.with_dupl, sampled_nbrs_per_node_dict, self._sampler.
                num_nodes, batch_dict.with_dupl, self.csc, self.disjoint)
            ...
            if self.disjoint:
                for ntype in node_types:
                    batch_dict.out[ntype], node_dict.out[ntype] = node_dict.out[ntype].t(
                    ).contiguous()
            ...
            node = OrderedSet(src.tolist()) if not self.disjoint else OrderedSet(
    ...
    def merge_sampler_outputs(
        ...
        cumm_sampled_nbrs_per_node = [o.metadata
        ...
        for p_id in partition_ids:
            ...
            start_edge = cumm_sampled_nbrs_per_node[p_id][
                p_counters[p_id] - 1] if p_counters[p_id] != 0 else 0
            p_counters[p_id] += 1
            ...
            batch.append(outputs[p_id].batch[start: end]
                         ) if self.disjoint else None
    ...
    async def _sample_one_hop(
        ...
        """
        src_ntype = (etype[0] if not self.csc else etype[2]
                     ) if etype is not None else None

        partition_ids = self.dist_graph.get_partition_ids_from_nids(
            srcs, src_ntype)

        p_outputs: List[SamplerOutput] = [
            None] * self.dist_graph.meta['num_parts']
        ...
        for i in range(self.dist_graph.num_partitions):
            ...
            if p_srcs.shape[0] > 0:
                if p_id == self.dist_graph.partition_idx:
                    ...
                    futs.append(rpc_async(to_worker, self.rpc_sample_callee_id, args=(
                        p_srcs, one_hop_num, p_seed_time, p_batch, etype)))
