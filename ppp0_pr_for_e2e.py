...
def run_training_proc(local_proc_rank: int, num_nodes: int, node_rank: int,
    ...
    # 1) partition -> graphstore/feature
    # get the graph/feature from the different partition  
    ...
    # get the partition related information
    (
    ...
    # 2) initialize the partition info into graphstore/featurestore 
    ...
    graph.labels = whole_node_labels
    
    # construct the graph/feature tuple for input to loader/sampler
    partition_data = (graph, feature)
    
 

    ## 3) global context info for rpc distributed/ ddp

    # dist_context for global world_size/world_rank/local_rank infos.
    ...
    print(f"----------- 111 -------------local_proc_rank={local_proc_rank}, current_ctx.rank={current_ctx.rank}, current_ctx.world_size={current_ctx.world_size}")

    
    
    ## 4) distributed loader/trainer part..
...
if __name__ == '__main__':
    ...
    f = open('dist_training_homo.txt', 'a+')
